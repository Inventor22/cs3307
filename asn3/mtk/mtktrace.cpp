...
//Added by Andrew Simpson on September 23, 2014
#include <iostream>
...
MTKMain::MTKMain()
{
   ...
   //   }
   //}
   // Andrew Simpson, September 22 2014:
   // Upated to always open this log.  Also removed output stating such.
	char psz1[] = "log.txt";
	pszFilename = psz1;
	flog = fopen(pszFilename, "a");
	if (!flog)
		fprintf(stderr, "# # # MTKTRACE ERROR: UNABLE TO WRITE: %s # # #\n", pszFilename);
	else {
...
//Added by AS on 2014-09-23:
bool mtkSetEnable(bool enable){
	return glblMTKAlive = enable;
}
...
bool mtkPrintLogFile(){
	if (glblMTKInst.flog != NULL){
		std::string strLine;
		//Move buffer pointer to the beginning of the file.to
		while (fscanf(glblMTKInst.flog, "%s", strLine) != EOF){
			printf("%s\n", strLine);
		}
		return true;
	}
